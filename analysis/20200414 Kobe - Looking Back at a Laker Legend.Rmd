...
![](assets/20200414/Kobe Bryant Picture.jpg)
...
On January 26, 2020, Kobe, his daughter Gianna, and seven others tragically lost their lives in a helicopter crash in Calabasas, California. 
...
source("./code/20200414/court_themes.R")
source("./code/20200414/plot_court.R")
source("./code/20200414/players_data.R")
source("./code/20200414/fetch_shots.R")
...
kobe_shots <- read.csv("./data/20200414/kobe_shots.csv")
...
![](assets/20200323/Kobe Bryant Shots 1996-1997.gif)

</div>

<div class = "col-md-6">

![](assets/20200323/Kobe Bryant Shots 2000-2001.gif)
...
![](assets/20200323/Kobe Bryant Shots 2007-2008.gif)

</div>

<div class = "col-md-6">

![](assets/20200323/Kobe Bryant Shots 2015-2016.gif)
...
<div class = "row">
```{r shots-zone-year-pct, fig.width=10, fig.height=8}
...
<div class = "row">
```{r shots-zone-year-pps, fig.width=10, fig.height=8}
...
```{r winning-time, eval=F}

winning_time <- kobe_shots %>%
  mutate(opponent = ifelse(htm == "LAL", as.character(vtm), as.character(htm)), 
         game_date = as.Date(game_date)) %>%
  group_by(game_id, opponent, game_date) %>% 
  dplyr::summarise(FGM = sum(shot_made_numeric), FGA = sum(shot_attempted_flag), 
                   PPS = round(mean(shot_made_numeric * shot_value), 4)) %>%
  ungroup() %>%
  filter(FGA >= 15) %>%
  arrange(desc(PPS)) %>%
  rename(Opponent = opponent, `Game Date` = game_date) %>%
  select(Opponent, `Game Date`, FGM, FGA, PPS) %>%
  head(n = 10)
